...
export default function AccountsPage() {
  ...
  const load = () => {
    axios.get(`${API}/accounts`, { headers })
      .then(r => { setGroup(r.data?.group ?? null); setAccounts(Array.isArray(r.data?.accounts) ? r.data.accounts : []); })
      .catch(() => { setGroup(null); setAccounts([]); })
    ...
    <AppLayout>
      <div className="p-4 sm:p-6 max-w-3xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl font-bold text-default">Cuentas</h1>
            <p className="text-sm text-soft">Sedes, sucursales y unidades de negocio.</p>
          </div>
        ...
        {/* Parent group */}
        {group && (
          <div className="bg-gradient-to-br from-ink-900 to-ink-700 text-white rounded-xl p-5 mb-4">
        ...
        {showForm && (
          <div className="surface rounded-xl border p-5 mb-4">
            <label className="text-xs text-soft block mb-1">Nombre de la cuenta</label>
            <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
              placeholder="Ej: Sede Bogotá" className="w-full surface border rounded-lg px-3 py-2 text-sm text-default mb-3" />
            <label className="text-xs text-soft block mb-1">Tipo</label>
        ...
        <div className="space-y-2">
          {accounts.length === 0 ? (
            <div className="surface rounded-xl border text-center py-12">
              ...
              <p className="text-sm text-soft">Sin cuentas hijas. Crea sedes o sucursales para organizar tu empresa.</p>
            </div>
          ) : accounts.map(a => (
            <div key={a.id} className="surface rounded-xl border p-4 flex items-center gap-3">
              <span className="text-2xl">{TYPE_ICON[a.type] ?? '🏢'}</span>
              <button onClick={() => router.push(`/accounts/${a.id}`)} className="flex-1 text-left">
                <p className="text-sm font-medium text-brand hover:underline">{a.name}</p>
                <p className="text-xs text-soft capitalize">{a.type} · {a.memberCount} usuario(s) · {a.status === 'active' ? 'activa' : 'inactiva'}</p>
              </button>
              {active === a.id ? (
                ...
                <button onClick={() => switchTo(a)} disabled={a.status !== 'active'}
                  className="text-xs surface-2 text-default px-3 py-1.5 rounded-lg hover:bg-surface-2 disabled:opacity-40">
                  Usar cuenta
                </button>
              )}
            </div>
