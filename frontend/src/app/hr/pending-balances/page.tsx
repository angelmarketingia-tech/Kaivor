    ...
    <AppLayout>
      <div className="p-4 sm:p-6 max-w-3xl mx-auto">
        ...
        <div className="flex items-center gap-2 mb-4 text-sm">
          ...
          <span className="text-soft">/</span><span className="text-default font-medium">Saldos pendientes</span>
        </div>

        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl font-bold text-default">Saldos pendientes</h1>
            {data?.summary && <p className="text-sm text-soft">{data.summary.pendingCount} pendiente(s) · {fmt(data.summary.pendingAmount)}</p>}
          </div>
        ...
        {showForm && (
          <div className="surface rounded-xl border p-5 mb-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label className="text-xs text-soft block mb-1">Empleado</label>
              ...
              <div>
                <label className="text-xs text-soft block mb-1">Monto</label>
                <input type="number" value={form.amount} onChange={e => setForm(f => ({ ...f, amount: e.target.value }))}
                  className="w-full border border-default rounded-lg px-3 py-2 text-sm bg-transparent text-default" />
              </div>
              <div>
                <label className="text-xs text-soft block mb-1">Fecha límite (opcional)</label>
        ...
        <div className="surface rounded-xl border overflow-hidden">
          {!data || data.balances.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-sm text-soft">Sin saldos registrados.</p>
            </div>
          ) : (
            <div className="divide-y divide-default">
              {data.balances.map((b: any) => (
                <div key={b.id} className="px-4 py-3 flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-default">{b.employee.firstName} {b.employee.lastName}</p>
                    <p className="text-xs text-soft">{TYPES[b.type] ?? b.type}{b.notes ? ` · ${b.notes}` : ''}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className={`text-sm font-bold ${b.status === 'pending' ? 'text-amber-600' : 'text-soft'}`}>{fmt(b.amount)}</span>
                    {b.status === 'pending' ? (
                      <>
                        ...
                        <button onClick={() => mark(b.id, 'cancelled')} className="text-xs text-soft hover:text-red-500">✕</button>
                      </>
                    ) : (
                      <span className="text-xs text-soft">{b.status === 'paid' ? 'Pagado' : 'Cancelado'}</span>
                    )}
                  </div>
                </div>
              ))}
