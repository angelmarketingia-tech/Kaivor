      ...
      <div className="p-6 max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">Gestiona todas tus facturas electrónicas</p>
          </div>
          <Link
        ...
        {stats && (
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-white rounded-xl border border-slate-200 p-4">
              ...
              <p className="text-xl font-bold text-slate-900">${stats.totalRevenue.toLocaleString('es-CO')}</p>
            </div>
            <div className="bg-white rounded-xl border border-slate-200 p-4">
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-4">
              <p className="text-xs text-slate-500 uppercase tracking-wide mb-1">Pendientes de cobro</p>
          ...
          {loading ? (
            ...
            <div className="text-center py-16 px-6">
              ...
              {filter === 'all' && (
                <Link
                  href="/invoices/create"
                  className="inline-block bg-slate-900 text-white px-5 py-2 rounded-xl text-sm font-medium hover:bg-slate-700 transition-colors"
                >
            ...
            <table className="w-full">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  <th className="text-left px-5 py-3 text-xs font-medium text-slate-500 uppercase tracking-wide">N° Factura</th>
                  <th className="text-left px-5 py-3 text-xs font-medium text-slate-500 uppercase tracking-wide hidden sm:table-cell">Cliente</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.map((inv) => {
                  ...
                  return (
                    <tr key={inv.id} className="hover:bg-slate-50 transition-colors cursor-pointer" onClick={() => router.push(`/invoices/${inv.id}`)}>
                      <td className="px-5 py-3.5 text-sm font-medium text-violet-700 hover:underline">{inv.invoiceNumber}</td>
