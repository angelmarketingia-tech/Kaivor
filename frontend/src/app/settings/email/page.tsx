    ...
    <AppLayout>
      <div className="p-4 sm:p-6 max-w-2xl mx-auto">
        ...
        <div className="mb-5">
          ...
          <h1 className="text-2xl font-bold text-default">Correo / SMTP</h1>
          <p className="text-sm text-soft mt-0.5">Configura tu correo para enviar facturas y recordatorios desde Kaivor.</p>
        </div>
        ...
        <div className="surface rounded-xl border p-5 mb-4">
          <p className="text-sm font-semibold text-default mb-2">Configuración rápida</p>
          <div className="flex gap-2 flex-wrap">
            {PRESETS.map(p => (
              <button key={p.name} onClick={() => applyPreset(p)}
                className="px-3 py-1.5 text-sm rounded-lg border border-default text-default hover:border-brand hover:bg-brand-50 transition-colors">
                {p.name}
              </button>
            ))}
          </div>
          {hint && <p className="text-xs text-brand mt-2">{hint}</p>}
        </div>

        {/* SMTP form */}
        <div className="surface rounded-xl border p-5 space-y-3">
          <h2 className="text-sm font-semibold text-default">Servidor SMTP</h2>
          <div className="grid grid-cols-3 gap-3">
            <div className="col-span-2">
              <label className="text-xs text-soft block mb-1">Host SMTP</label>
              <input type="text" value={host} onChange={e => setHost(e.target.value)}
                placeholder="smtp.gmail.com"
                className="w-full surface border rounded-lg px-3 py-2 text-sm text-default" />
            </div>
            <div>
              <label className="text-xs text-soft block mb-1">Puerto</label>
              <input type="number" value={port} onChange={e => setPort(e.target.value)}
                className="w-full surface border rounded-lg px-3 py-2 text-sm text-default" />
            </div>
          </div>
          <div>
            <label className="text-xs text-soft block mb-1">Usuario / email</label>
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-sm text-default">Conexión segura SSL/TLS (recomendado para puerto 465)</span>
          </label>
        </div>

        {/* Sender */}
        <div className="surface rounded-xl border p-5 space-y-3 mt-4">
          <h2 className="text-sm font-semibold text-default">Remitente</h2>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-soft block mb-1">Nombre del remitente</label>
              <input type="text" value={fromName} onChange={e => setFromName(e.target.value)}
                placeholder="Mi Empresa S.A.S."
                className="w-full surface border rounded-lg px-3 py-2 text-sm text-default" />
            </div>
            <div>
              <label className="text-xs text-soft block mb-1">Email del remitente</label>
        ...
        <div className="flex gap-2 mt-4">
          ...
          <button onClick={testConnection} disabled={testing || !settings?.emailConfigured}
            className="px-4 py-2.5 rounded-xl text-sm font-medium border border-default text-default hover:bg-brand-50 disabled:opacity-40">
            {testing ? 'Enviando…' : 'Probar conexión'}
          </button>
        </div>
        {!settings?.emailConfigured && (
          <p className="text-xs text-soft mt-2 text-center">Guarda la configuración antes de probar la conexión.</p>
        )}
      </div>
    </AppLayout>
  );
